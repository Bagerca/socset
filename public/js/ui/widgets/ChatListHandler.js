...
export class ChatListHandler {
    ...
    togglePin(chatId) {
        if (this.pinnedChats.includes(chatId)) {
            this.pinnedChats = this.pinnedChats.filter(id => id !== chatId);
        } else {
    ...
    renderChats() {
        ...
        const sorted = [...filtered].sort((a, b) => {
            return (this.pinnedChats.includes(b.id) ? 1 : 0) - (this.pinnedChats.includes(a.id) ? 1 : 0) || b.updated_at - a.updated_at;
        });
    ...
    bindEvents() {
        ...
        if (this.chatSearchInput) {
            this.chatSearchInput.addEventListener('input', (e) => handleSearchInput(e.target.value.trim()));
        ...
        document.addEventListener('click', (e) => {
            ...
            if (item && this.chatListContainer.contains(item)) {
                this.onChatSelected(item.dataset.id);
                return;
            }
            ...
            if (dropItem) {
                this.onChatSelected(dropItem.dataset.id);
                return;
            }
