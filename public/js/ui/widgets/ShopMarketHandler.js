...
export class ShopMarketHandler {
    ...
    render() {
        const items = this.stores.shop.items;
    ...
    bindEvents() {
        ...
        const handleSearch = debounce((query) => {
            ...
            const results = this.searchEngine.search(this.stores.shop.items, query, [{ field: 'name', weight: 3 }, { field: 'author', weight: 1 }]);
            if (results.length > 0) {
        ...
        this.grid.addEventListener('click', async (e) => {
            ...
            if (buyBtn && !buyBtn.classList.contains('bought')) {
                ...
                if (success) {
                    ...
                    this.render(); // Перерисовываем сетку (кнопка станет "В инвентаре")
                } else {
