...
export class PostCommentHandler {
    ...
    _renderComments() {
        ...
        // GPU ОПТИМИЗАЦИЯ: requestAnimationFrame для дерева комментов
        const html = PostRenderer.renderCommentsTree(this.post.comments, this.stores);
    ...
    bindEvents() {
        ...
        }, { passive: true }); // GPU ОПТИМИЗАЦИЯ: passive true
        
        this.container.addEventListener('touchend', () => clearTimeout(pressTimer), { passive: true });
        this.container.addEventListener('touchmove', () => clearTimeout(pressTimer), { passive: true });
