...
class MessagesController {
    ...
    getChats(req, res) {
        const username = req.user.username;
        try {
            const chats = db.prepare(`SELECT c.* FROM chats c JOIN chat_members cm ON c.id = cm.chat_id WHERE cm.username = ? ORDER BY c.updated_at DESC`).all(username);
    ...
    getChatDetails(req, res) {
        ...
        try {
            ...
            const mediaMessages = db.prepare(`SELECT content FROM messages WHERE chat_id = ? AND content LIKE '[IMG:%' ORDER BY timestamp DESC`).all(chatId);
